...
先算上界。适配器的参数量是 r·Σ_l(d_l + k_l)，反过来就是 r ≤ 预算 / Σ_l(d_l + k_l)，一个除法。Llama-3-8B 的规模（32 层、隐藏 4096、FFN 14336、7 个线性层）Σ(d+k) 约 2.8M：预算给到全参的 0.5%（约 45M 参数），除下来 r ≈ 16。Biderman et al. 2024 说的“按显存约束挑 r”就是这个意思——在这个范围里，r 是成本调节，16 配全部模块就是他们试过的配置里给的推荐。
...
训完用参与比收尾：远小于 r 就缩，奇异值尾部还大就加。手调的逐层配置通常不抵复杂度。所有数字最后都在 held-out 上过一遍——两三个点。
