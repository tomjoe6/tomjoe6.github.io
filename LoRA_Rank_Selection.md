...
r 没有可以直接算出来的公式。谱回答的不是 r 是多少，而是这个任务上 LoRA 够不够用；能算出来的只有上界，按你愿意付的参数量卡。真正定 r 的那一步只能测——在 held-out 上取最小的 r，使指标达到全参的九成，两三个点就够。
...
先算上界。适配器的参数量是 r·Σ_l(d_l + k_l)；覆盖全部线性层时，你能接受的参数量一确定，r 的上界就出来了。Llama-3-8B 的规模（32 层、隐藏 4096、FFN 14336、7 个线性层）Σ(d+k) 约 2.8M，r = 16 对应约 45M 参数、全参的 0.5% 上下。Biderman et al. 2024 说的“按显存约束挑 r”就是这个意思——在这个范围里，r 是成本调节，16 配全部模块就是他们试过的配置里给的推荐。
...
此外，所需的秩取决于数据量。Biderman et al. 2024 在持续预训练里把数据从 0.25B 加到 20B token，同一层 ΔW 解释到九成方差所需的秩随数据量涨；任务离预训练分布越远，需要的维度越高。
