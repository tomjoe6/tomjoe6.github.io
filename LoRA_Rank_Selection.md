...
起点用默认值：全部线性层、r = 16，α 同步取 2r。Biderman et al. 2024 试过的配置里，这就是他们的建议——按显存约束挑 r，16 配全部模块是个好起点。

要更准，就把 r 定在 held-out 达到全参九成的那一点（式 6）：有全参 ckpt 就冻结随机 A、只训 B；没有就把更新限制在随机 d 维子空间里扫。早期谱代理能先给个量级，但只是粗估。
